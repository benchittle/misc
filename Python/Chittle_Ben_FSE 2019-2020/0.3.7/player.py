...
Added:
    >Player.drop_item
    >Player.update_item_position *from sprites.HandBlaster*

Removed:

Changed:
    >Player.pickup_item - updates the image of the corresponding menu button
        when a new item is picked up
    >Player.pickup_item - now tries to pick up an item with a small radius of the
        mouse's position, rather than the player's
...
class Player(sprites.ShipSprite):
    ...
    noclip = False
    def __init__(self, lvldata, ship, pos):
        super().__init__()
        ...
        self.inventory[0] = sprites.HandBlaster.from_owner(self) ##
