...
class Button:
    def __init__(self, menu, rect, image, rel_pos, index):
        ...
        self.index = index
        self.image_default = pg.transform.scale(
            image,
            (self.rect.w - config.HOTBAR_OFFSET * 2, self.rect.h - config.HOTBAR_OFFSET * 2))
        self.image_pop = pg.transform.scale(self.image_default, (self.rect.w, self.rect.h))
        self.image = self.image_default ##simplify??
