...
Version: 0.3.4
Added:
    'colour' argument to HandBlaster.__init__
    HandBlaster.colour
    ...
    def pickup_item(self):
        ...
        if items:
            try:
                ...
                self.inventory[self.inventory.index(None)] = new_item
                new_item.owner = self
                new_item.is_on_ground = False
                if self.inventory:
                    new_item.is_hidden = True
                else:
                    new_item.is_hidden = False
                    self.equipped_item = self.inventory.index(new_item)
...
class HandBlaster(pg.sprite.Sprite):
    ...
    def __init__(self, game, ship, pos, rotation=0, colour=config.RED):
        super().__init__()
        self.game = game
        self.ship = ship
