...
class Player(sprites.ShipSprite):
    size = (config.PLAYER_SIZE, config.PLAYER_SIZE)
    hitbox_size = (math.sqrt(2) * config.PLAYER_SIZE,) * 2
    noclip = False
    def __init__(self, lvldata, pos):
        ...
        self.item_pos_offset = (config.PLAYER_SIZE / 2, config.PLAYER_SIZE / 2)
        self.inventory = [None] * 9
        self.inventory[0] = items.HandBlaster.from_owner(self, items.RegularShot) ##
    ...
    def update_item_position(self):
        ...
        radial_vector = item.pos - self.pos
    ...
    def draw_hitbox(self):
        hitbox = pg.Rect((0, 0), self.hitbox_size)
        hitbox.center = self.rect.center
        pg.draw.rect(self.lvldata.screen, config.RED, hitbox, 1)
    ...
    def primary_action(self):
        item = self.equipped_item
        if item is not None:
            if pg.time.get_ticks() - item.last_shot_time > item.cooldown:
                trajectory = pg.Vector2(pg.mouse.get_pos()) - item.rect.center
                item.action(trajectory)
