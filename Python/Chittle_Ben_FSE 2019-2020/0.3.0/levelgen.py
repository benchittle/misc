...
class TileGrid(list):
    def __init__(self, sub, rows, columns):
        ...
        prev_row_span = self.columns // 2 - 1#random.randint(1, self.columns // 2 - 1)

        # Generate a semi-random number of tiles centered in each row of the
        # grid.
        for row in range(self.rows):
            if left_mid - prev_row_span <= 0 or right_mid + prev_row_span >= self.columns - 1:
                row_span = prev_row_span - random.randint(0, 1)
            else:
                row_span = prev_row_span + random.randint(-1, 1)
...
class Room(pg.sprite.Sprite):
    ...
    def build(self):
        ...
        for loc, Kind in self.sides.items():
            Kind(self.sub.game, self.placements[loc], self.sub.game.walls)
...
class Corridor(pg.sprite.Sprite):
    ...
    def build(self):
        ...
        for loc, Kind in self.sides.items():
            ...
            Kind(self.sub.game, self.placements[loc], self.sub.game.walls, anchor=anchor)
