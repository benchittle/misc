...
Changed:
    >TileGrid.iter_rows -> TileGrid.filtered_rows
    >TileGrid.iter_rows -> TileGrid.filtered_cols
    >TileGrid.iter_tiles - modified to use TileGrid.filtered_rows method
    >Room and Corridor 'rect' attributes are now pointers to their parent tile's
        rect
    >'Sub' class was renamed to 'Ship'
        >attributes named 'sub' became 'ship'
"""
...
class TileGrid(list):
    ...
    def iter_tiles(self):
        """Iterate by row over each tile the filtered matrix."""
...
class Ship:
    def __init__(self, game, pos, max_width, length):
        ...
        self.pos = self.game.screen_pos + self.rect.topleft
    ...
    def update(self):
        self.rect.topleft = self.pos - self.game.screen_pos
...
class Tile(pg.sprite.Sprite):
    def __init__(self, ship, row, col, *tags):
        super().__init__()
        ship.game.all_sprites.add(self, layer=config.TILE_LAYER)
        self.ship = ship
        self.row = row
        self.col = col
        ...
        self.rect.topleft = self.pos - self.ship.game.screen_pos
...
class Room(pg.sprite.Sprite):
    def __init__(self, tile):
        super().__init__()
        tile.ship.game.all_sprites.add(self, layer=config.ROOM_LAYER)
        self.ship = tile.ship
        self.tile = tile
    ...
    def build(self):
        ...
        for loc, Kind in self.sides.items():
            Kind(self.ship.game, self.placements[loc], self.walls)
...
class Corridor(pg.sprite.Sprite):
    ...
    def __init__(self, tile):
        ...
        self.ship = tile.ship
        self.tile = tile
        self.tile.ship.game.all_sprites.add(self, layer=config.ROOM_LAYER)
    ...
    def build(self):
        ...
        for loc, Kind in self.sides.items():
            ...
            Kind(self.ship.game, self.placements[loc], self.walls, anchor=anchor)
