...
Added:
    >ShipSprite
        >ShipSprite.tiles (attr with setter and getter)
        >ShipSprite.get_nearby_tiles
        >ShipSprite.get_walls

Removed:
    >spritecollide_sub - Sub.locate_sprite now uses this algorithm algorithm

Changed:
    >Player.update_position - collision detection changed to use more optimized
        methods from ShipSprite class
    >Blaster.update_position - collision detection changed to use more optimized
        methods from ShipSprite class
    >Blaster.update_position_bouncy - collision detection changed to use more
        optimized methods from ShipSprite class
    >'sub' -> 'ship'
    >Player - 'ship' is now a parameter for creating instances
    >HandBlaster - 'ship' is now a parameter for creating instances
"""
...
class ShipSprite(pg.sprite.Sprite):
    ...
    @property
    def tiles(self):
        """'tiles' getter function."""
        return self._tiles

    @tiles.setter
    def tiles(self, tiles):
        """'tiles' setter function"""
        ...
        if self.tiles != tiles:
            ...
            # Update the list of Blocks the sprite could be colliding with.
            self._walls_to_check = self.get_walls()

        # If 'tiles' is an empty list.
        elif not tiles:
