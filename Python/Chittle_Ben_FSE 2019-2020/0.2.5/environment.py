...
class Block(pg.sprite.Sprite):
    def __init__(self, game, pos, dimensions, *groups, anchor="topleft"):
        ...
        self.colour = config.WALL_COLOUR#[255] + [random.randint(0, 255) for i in range(2)]
    ...
    def split(self, gap, axis, kill=True): #use getattr?
        if axis == "x": #vertical
...
'''
class VConnector(Composite):
    def __init__(self, game, pos, anchor="topleft", groups=()):
        super().__init__(game, pos, anchor)
        size = (config.WALL_WIDTH, 2 * config.WALL_WIDTH + config.DOOR_SIZE)
        temp_wall = Block(self.game, self.pos, size, self.anchor)
...
class HConnector(Composite):
    def __init__(self, game, pos, anchor="topleft", groups=()):
        super().__init__(game, pos, anchor)
        size = (2 * config.WALL_WIDTH + config.DOOR_SIZE, config.WALL_WIDTH)
        temp_wall = Block(self.game, self.pos, size, self.anchor)
